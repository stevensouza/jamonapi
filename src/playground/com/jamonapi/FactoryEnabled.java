...
public class FactoryEnabled implements MonitorFactoryInterface {
    ...
    private static final RangeImp RANGE_NULL_OBJ=new NullRange();
   ...
   public String getVersion() {
    ...
    private MonitorImp getTimeMonitor(MonKey key, boolean isPrimary) {
       MonitorImp mon=getMonitor(key, isPrimary, TIME_MONITOR);
       // Note this check isn't strictly required but it saves the object
       // creation and other logic if the monitor is disabled.  However,
       // if mon was used then it would work just as well other than that.
       if (mon.isEnabled())
 //  jamon 2.4 ???????????? fix        return new TimeMon(mon);
         return new TimeMon(key, mon);
       else
         return mon;
    }
 ...
 //   private MonitorImp getMonitor(MonKey key, boolean isPrimary, boolean isTimeMonitor) {
    private synchronized MonitorImp getMonitor(MonKey key, boolean isPrimary, boolean isTimeMonitor) {
      ...
      if (mon==null) { //??? needs to be synchronized.
        mon=createMon(key, isPrimary, isTimeMonitor);
        putMon(key, mon);          
      }
      
      // make more inline with TimeMonitor way of creating 
      // jamon 2.4 ??????
//      if (!isTimeMonitor)
//    	 mon=new DecoMon(key, mon);
...
//jamon 2.4 comment out          MonitorImp mon=new BaseMon(new Mon(key, range, activityStats, isTimeMonitor)); 
//          MonitorImp mon=null;
//          if (isTimeMonitor)
//        	mon=new TimeMon(key, range, activityStats, isTimeMonitor);
//          else 
//            mon=new Mon(key, range, activityStats, isTimeMonitor); 
          MonitorImp mon=new Mon(key, range, activityStats, isTimeMonitor); 
